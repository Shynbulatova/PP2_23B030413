...
DISAPPEAR_TIME = random.randint(10000, 10000)  # Random time 
...
food = (2, 2)
food_size = random.randint(1, 1) * CELL_SIZE  # Random size between 1 and 3 times the cell size
...
conn = psycopg2.connect(host="localhost", dbname="Amina", user="postgres",
                       password="Taehyung", port="5432")
cur=conn.cursor()
cursor = conn.cursor()
...
def get_user_level(user_id):
    cursor.execute('SELECT level FROM user_scores WHERE user_id = %s ORDER BY id DESC LIMIT 5', (user_id,))
